...
/**
 * The read side of the fleet the `benzene:mesh:query:*` handlers answer - the seam that makes the fleet UI's data
...
export interface IMeshFleetReadModel {
  /**
   * The whole known fleet - services, topic catalog, recent flows (`benzene:mesh:query:fleet`). An optional `range`
  ...
  /**
   * One service's detail, or `undefined` if unknown (`benzene:mesh:query:service`). Optional `range` windows the
  ...
  /**
   * One topic's summary, or `undefined` if unknown (`benzene:mesh:query:topic`). Optional `range` windows the
  ...
  /**
   * One flow's events in start order -> the waterfall, or `undefined` if unknown (`benzene:mesh:query:trace`). A
  ...
  /**
   * Every flow that carried a business correlation id, or `undefined` if none (`benzene:mesh:query:correlation`).
