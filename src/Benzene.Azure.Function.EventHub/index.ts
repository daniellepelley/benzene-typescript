...
// deliberately DIFFERENT in shape from Service Bus/Kafka: there are no per-message body/topic/headers
// getters or result setter. Instead `BenzeneMessageEventHubHandler` (a `MiddlewareRouter`) deserializes
// each event body into a `BenzeneMessageRequest`, routes it through the transport-agnostic
// `BenzeneMessageApplication`, and surfaces the inner result onto `EventHubContext.messageResult` (which
// `EventHubOptions.raiseOnFailureStatus` escalation reads) — a faithful port of the C#
// `Benzene.Azure.Function.EventHub.Function` package as it actually exists.
//
