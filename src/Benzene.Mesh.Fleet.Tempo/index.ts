 ...
 * (alongside `@benzene/mesh-fleet-jaeger` and the X-Ray adapter): it answers the mesh's `mesh:query:trace`,
 ...
 *
 * Not to be confused with `@benzene/mesh-tracing-tempo`: that sibling reads Tempo's **metrics-generator**
 ...
 * snapshot of `@benzene/mesh-wire`'s `MeshTraceEvent` has no such field.
 */
