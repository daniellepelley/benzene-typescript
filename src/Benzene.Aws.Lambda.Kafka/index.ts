...
export * from './KafkaBatchFailureMode';
export * from './KafkaBatchResponse';
export * from './KafkaBatchProcessingException';
...
// (the same choice the SQS/SNS adapters made), plus a hand-modeled `KafkaBatchResponse` (no such type in
// `@types/aws-lambda`). STRUCTURAL note: `event.records` is an OBJECT keyed by `"topic-partition"` →
// `MSKRecord[]` (processed per-partition, sequentially in offset order, by `KafkaApplication`);
// `record.value` is BASE64 (decoded by the body getter); `record.headers` is an array of
// `{ [key]: number[] }` byte-array batches.
//
