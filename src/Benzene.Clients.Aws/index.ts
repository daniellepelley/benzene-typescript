 ...
 *
 * Re-exports the outbound broker route extensions and their clients / reachability checks for SNS, SQS,
 * EventBridge, Step Functions, and the low-level AWS Lambda invoke client — `useSns` / `useSqs` /
 * `useEventBridge` / `useStepFunctions` and their `use*Client` / `add*HealthCheck` companions. Depend on
 * this package for the whole AWS outbound-client surface, or on the individual `@benzene/clients-aws-*`
 * packages for a narrower dependency.
 *
