/**
 * Lambda entry point for the orders-api Cloud Service — POST /orders over API Gateway; sends payments:capture (SQS) and order:placed (SNS) at runtime.
 *
 ...
 * supplied by `productionOutbound`. Terraform points this function's `handler` at `orders.handler`.
 */
...
const definition = serviceDefinition('orders-api');
