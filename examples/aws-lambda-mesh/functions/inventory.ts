/**
 * Lambda entry point for the inventory-api Cloud Service — a pure event consumer (order:placed over SNS, shipment:dispatched over EventBridge).
 *
 ...
 * supplied by `productionOutbound`. Terraform points this function's `handler` at `inventory.handler`.
 */
...
const definition = serviceDefinition('inventory-api');
