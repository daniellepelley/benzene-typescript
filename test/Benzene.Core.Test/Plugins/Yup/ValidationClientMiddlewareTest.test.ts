 ...
 * Benzene.FluentValidation.ValidationClientMiddleware, ADAPTED to Yup. The schema is resolved from the
 * message instance's constructor (the erasure-driven client-side counterpart of C#'s
 * `IValidator<TRequest>` DI resolution). Yup validates asynchronously and by throwing, unlike
 * Joi/Zod — the middleware handles that, so these tests read identically.
 */
...
registerYupSchema(ClientRequest, yup.object({ name: yup.string().max(10) }));
